...
namespace HoloToolkit.Unity {
    ...
    public class SpatialUnderstanding : Singleton<SpatialUnderstanding> {
        ...
        public List<MeshFilter> GetMeshFilters() {
            ...
            AddSceneContentFillter(SceneContent, ref meshes);
            return meshes;
        }

        public void AddSceneContentFillter(GameObject obj, ref List<MeshFilter> meshes) {
            if (obj.GetComponent<MeshFilter>()) {
                meshes.Add(obj.GetComponent<MeshFilter>());
                if (obj.transform.childCount != 0) {
                    for (int i = 0; i < obj.transform.childCount; i++) {
                        GameObject child = obj.transform.GetChild(i).gameObject;
                        AddSceneContentFillter(child, ref meshes);
                    }
                }
            }
            else {
